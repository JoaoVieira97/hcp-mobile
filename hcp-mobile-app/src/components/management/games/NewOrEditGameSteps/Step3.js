...
class Step3 extends Component {
    ...
    componentDidMount() {
        ...
        if(this.props.newOrEditGame.rawCoachesIDs.length > 0) {

            this.props.setStepDisabled(false);
            this.setState({coaches: true});

            const allCoaches = this.props.newOrEditGame.allCoaches.map(item => {
                if(this.props.newOrEditGame.rawCoachesIDs.includes(item.id)) {
                    item.visible = false;
                }
                return item;
            });
    ...
    handleRemoveCoach = async (value) => {
        ...
        if(this.props.newOrEditGame.rawCoachesIDs.length === 1){
    ...
    renderItem = ({ item }) => {
        ...
        const coachesFiltered = this.props.newOrEditGame.allCoaches.filter(coach => coach.id === item);
    ...
    render() {
        ...
        const coachesFiltered = this.props.newOrEditGame.allCoaches.filter(item => item.visible);
        ...
        let firstTitle;
        const size = this.props.newOrEditGame.rawCoachesIDs.length;
        if (size === 1)
            firstTitle = (
            ...
            <React.Fragment>
                ...
                <Animatable.View style={{margin: 20}} animation={"fadeIn"}>
                    <Card elevation={6}>
                        <Card.Title
                            title="Treinadores"
                            subtitle="Adicione treinadores a este jogo."
                            left={(props) =>
                            ...
                            <View>
                                <View style={{
                                    ...
                                    <FlatList
                                        ...
                                        data={this.props.newOrEditGame.rawCoachesIDs}
